...
public class GameSystem {
	...
	public void action(Piece piece, boolean attack, int actionPattern) {
		if (piece instanceof MotherShip) {
			MotherShip m = (MotherShip) piece;
			if (attack) {
				m.attackTargetPosition(actionPattern);
				m.move(0);
				removeOtherPieces(m, m.getCurrentPosition());
			} else {
				m.move(actionPattern);
			}
		} else if (piece instanceof Fighter) {
			Fighter f = (Fighter) piece;
			f.attackTargetPosition(actionPattern);
			boolean remove = removeOtherPieces(f, f.getCurrentPosition());
			if (remove) {
				removeOtherPieces(f, f.getCurrentPosition());
				f.setPromoted(true);
			} else {
				f.move(actionPattern);
			}
